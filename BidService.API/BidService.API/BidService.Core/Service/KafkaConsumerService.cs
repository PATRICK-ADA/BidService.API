...
namespace BidService.API.KafkaConsumerService
{
    public class KafkaConsumerService : BackgroundService
    {
        ...
        public KafkaConsumerService(IConfiguration configuration, IServiceScopeFactory serviceScopeFactory, ILogger logger)
        {
        ...
        private async Task ProcessMessage(string messageValue)
        {
            ...
            if (bidMessage != null)
            {
                var existingBid = await context.RoomBidders
                    .AsNoTracking()
                ...
                if (existingBid == null)
                {
                    await context.RoomBidders.AddAsync(bidMessage);
                    await context.SaveChangesAsync();
                }
            }

            var Bidders = await context.RoomBidders.ToListAsync();

            var tasks = Bidders.Select(async h =>
            {
                var message = JsonConvert.SerializeObject(h);
                await _producer.ProduceAsync("Bid-Topic", new Message<string, string> { Key = h.UserName, Value = message });
                _logger.Information("Published Bidders to Kafka.");
            });
